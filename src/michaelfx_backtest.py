...
def run_branch(sym, p, month_start):
    ...
    trades = []; open_tr = None; day_count = {}; warm = 60
    for t in range(warm, n):
        ...
        if open_tr:
            ...
            if hit is not None:
                ...
                open_tr = None
            continue
        # buscar entrada (a favor del sesgo, en sesión, respetando 2/día)
        day = pd.Timestamp(dts[t]).date()
        if p["sessions"] and not _in_session(dts[t]):
            continue
        if day_count.get(day, 0) >= 2:
            continue
        b = bias_at[t] if p["use_bias"] else 0
        # LONG: mitiga OB alcista (demanda) y sesgo alcista/neutral
        if (not p["use_bias"] or b == 1):
            for j, (top, bot) in enumerate(bull):
                if l[t] <= top and c[t] >= bot:            # precio entra a la zona
                    entry = top; sl = bot - p["buf"]*(top-bot); risk = entry-sl
                    if risk <= 0: continue
                    open_tr = {"side": 1, "entry": entry, "sl": sl, "tp": entry + p["rr"]*risk, "bars": 0}
                    bull.pop(j); day_count[day] = day_count.get(day, 0)+1; break
        if open_tr: continue
        if (not p["use_bias"] or b == -1):
            for j, (top, bot) in enumerate(bear):
                if h[t] >= bot and c[t] <= top:
                    entry = bot; sl = top + p["buf"]*(top-bot); risk = sl-entry
                    if risk <= 0: continue
                    open_tr = {"side": -1, "entry": entry, "sl": sl, "tp": entry - p["rr"]*risk, "bars": 0}
                    bear.pop(j); day_count[day] = day_count.get(day, 0)+1; break
...
BRANCHES = {
    "A_baseline":   dict(use_bias=True,  bias_tf="H1", rr=2.5, sessions=True,  ob_str=5, buf=0.1, max_hold=48),
    "B_rr5":        dict(use_bias=True,  bias_tf="H1", rr=5.0, sessions=True,  ob_str=5, buf=0.1, max_hold=48),
    "C_no_session": dict(use_bias=True,  bias_tf="H1", rr=2.5, sessions=False, ob_str=5, buf=0.1, max_hold=48),
    "D_bias4H":     dict(use_bias=True,  bias_tf="H4", rr=2.5, sessions=True,  ob_str=5, buf=0.1, max_hold=48),
    "E_no_bias":    dict(use_bias=False, bias_tf="H1", rr=2.5, sessions=True,  ob_str=5, buf=0.1, max_hold=48),
}
...
def main():
    ...
    for bname, p in BRANCHES.items():
        ...
        if tot:
            print(f"\n[{bname}]  TOTAL: n={tot['n']} wr={tot['wr']}% expR={tot['expectancy']:+.2f} ΣR={tot['totalR']:+.1f}")
            print("   por par: " + " · ".join(f"{s}(n{v['n']} {v['expectancy']:+.2f}R)" for s, v in per.items()))
        else:
            print(f"\n[{bname}]  sin trades este mes")
